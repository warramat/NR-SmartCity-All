liff.init({ liffId: '2000414439-BvmeyznD' }).then(async () => {
  ...
  if (!liff.isLoggedIn()) {
    liff.login({
      redirectUri: `https://nr-smartcity-all.netlify.app/compailn/check_details.html?key=${key}&_id=${_id}`
    });
  ...
  if (Star == 0) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const _id = urlParams.get('_id');
        let url =
          'https://nr-api-smartcity-final.onrender.com/appeal/addStar/Comment/' +
          _id;
...
window.onload = async () => {
  const img_url = 'https://nr-api-smartcity-final.onrender.com/userSmart/';
  const UID = await getUID();
  const urlParams = new URLSearchParams(window.location.search);
  const key = urlParams.get('key') || 0;
  const _id = urlParams.get('_id');
  let data = await fetch(
    'https://nr-api-smartcity-final.onrender.com/appeal/find/data?userID=' + UID
  );
